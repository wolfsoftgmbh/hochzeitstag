document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateTimer() {
        ...
        if (elMilestoneList) {
            ...
            // Update Next Email Display
            const elNextEmailInfo = document.getElementById('next-email-info');
            if (elNextEmailInfo && CONFIG.nextEmail && uniqueMilestones.length > 0) {
                let nextEmailDate = null;
                let triggerReason = "";
                
                // Sort all triggers for all milestones
                const triggers = [];
                uniqueMilestones.forEach(m => {
                    // Trigger 1: 7 days before
                    const t1 = new Date(m.date);
                    t1.setDate(t1.getDate() - 7);
                    triggers.push({ date: t1, label: m.label, type: '7 Tage vorher' });
                    
                    // Trigger 2: 1 day before
                    const t2 = new Date(m.date);
                    t2.setDate(t2.getDate() - 1);
                    triggers.push({ date: t2, label: m.label, type: '1 Tag vorher' });
                });
                
                // Sort triggers by date
                triggers.sort((a,b) => a.date - b.date);
                
                // Find first trigger in future (or today)
                // Normalize "now" to midnight for comparison
                const todayMidnight = new Date(now);
                todayMidnight.setHours(0,0,0,0);
                
                for (const t of triggers) {
                    if (t.date >= todayMidnight) {
                        nextEmailDate = t.date;
                        triggerReason = `${t.type} (${t.label})`;
                        break;
                    }
                }
                
                if (nextEmailDate) {
                    elNextEmailInfo.style.display = 'block';
                    elNextEmailInfo.innerText = `Nächste E-Mail: ${formatShortDate(nextEmailDate)} (${triggerReason})`;
                }
            }
        }
    }
